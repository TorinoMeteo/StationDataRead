	...
	for (sql_id, sql_name, sql_data_url, sql_dateformat, sql_timeformat, sql_data_type_id) in output_data:	
		...
		if sql_data_type_id == 1:
			...
			if sql_timeformat == None:
				sql_timeformat = '%H:%M'
			try:
				data = url.urlopen(url=str(sql_data_url),timeout=10000).read(20000)
